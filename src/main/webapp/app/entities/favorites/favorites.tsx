...
export const Favorites = () => {
  ...
  return (
    <div>
            ...
            <tbody>
              {favoritesList.map((favorites, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{favorites.customer ? <Link to={`/customer/${favorites.customer.id}`}>{favorites.customer.id}</Link> : ''}</td>
                  <td>{favorites.post ? <Link to={`/post/${favorites.post.id}`}>{favorites.post.id}</Link> : ''}</td>
                  <td className="text-end">
                    <div className="btn-group flex-btn-group-container">
                      ...
                      <Button
                        tag={Link}
                        to={`/favorites/${favorites.id}/edit?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="primary"
                        size="sm"
                      ...
                      <Button
                        tag={Link}
                        to={`/favorites/${favorites.id}/delete?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="danger"
                        size="sm"
      ...
      {totalItems ? (
        <div className={favoritesList && favoritesList.length > 0 ? '' : 'd-none'}>
